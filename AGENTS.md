...
- Release Please runs from `.github/workflows/release.yml` and uses Conventional Commits to create versioned release PRs, tags, and GitHub Releases. `.github/workflows/binaries.yml` publishes macOS ARM64/x64 and Linux ARM64/x64 assets plus `SHA256SUMS`, then updates the Homebrew tap; see `RELEASING.md`.
...
- Secrets and shared variables live in a plain `.env` in the collection root (gitignored; `collection/.env.example` documents the fixture values). Plain `.env` is not an environment; `environmentNames()` only matches `.env.<suffix>` and skips `*.example`.
...
- `cd wiremock && docker compose up -d` starts a deterministic API on `http://localhost:3000` used by the `dev` environment. Login captures `dev-token`; user endpoints require it as a bearer token.
